...
//! else. There is no server and no synchronization, so this crate is the whole persistence
//! story.
//!
...
//!    "reset application data" are therefore different operations that cannot be confused.
//! 2. **Library rows denormalize what they display.** History and playlists keep their own copy of
//!    the title, channel and thumbnails, so they render offline and after a cache clear.
